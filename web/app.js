...
const draftFields = ['title','style','lyrics','brief','language','cot','seed','cfg','maxTokens','abc','genre','tempo','bpm','vocal','mood','melodyFocus','referenceTrack','engine','duration','outputFormat','sourceSongId','sourceTitle','derivationMode','referenceStrength'];
...
function engineLabel(engine){return {'mixed':'モデル混在','yue2':'YuE2','ace-xl-turbo':'ACE-Step','stable-audio-3-medium':'Stable Audio 3'}[engine]||engine;}
function engineFields(){const auto=isTimedEngine()&&$('duration').value==='auto'&&$('derivationMode').value!=='cover';return {engine:$('engine').value,duration:auto?180:Number($('duration').value)||60,duration_mode:auto?'auto':'fixed'};}
function updateMode(){
  const stable=$('engine').value==='stable-audio-3-medium';
  $('vocal').disabled=stable;
  if(stable)$('vocal').value='instrumental';
  updateDerivative();
  const automaticDuration=isTimedEngine()&&$('duration').value==='auto'&&$('derivationMode').value!=='cover';
  ...
  $('lyrics').disabled=inst||$('derivationMode').value==='score';$('lyrics').required=!inst&&!$('autoAssist').checked;$('style').required=!inst&&!$('autoAssist').checked;
...
function updateDerivative(){const mode=$('derivationMode').value,active=!!$('sourceSongId').value&&mode!=='none';$('derivativeBox').hidden=!active;$('engine').disabled=active;$('sourceSongTitle').textContent=active?`元曲：${$('sourceTitle').value}`:'';$('referenceStrengthBox').hidden=mode!=='cover';$('duration').disabled=active&&mode==='cover';$('bpm').disabled=active&&['score','cover'].includes(mode);$('tempo').disabled=$('bpm').disabled;$('derivativeHint').textContent=mode==='score'?'元の譜面と歌詞を引き継ぎ、音色・編成・歌い方を変えます。完成音声の声や演奏が完全に同一になる保証はありません。':mode==='cover'?'元音源のメロディーと構成を足場に別テイクを生成します。長さは元音源に従います。保持の強さは再現率ではありません。':'元音源の音色やミックスの雰囲気を参照します。旋律・コード・構成は新しく生成されます。';}
$('derivationMode').addEventListener('change',()=>{const mode=$('derivationMode').value;if(mode==='none'){clearDerivative();return}const id=$('sourceSongId').value;startDerivative(id,mode,true);});
function clearDerivative(){$('sourceSongId').value='';$('sourceTitle').value='';$('derivationMode').value='none';$('abc').value='';updateMode();saveDraft();}
